...
# Custom function to calculate accuracy within defined price ranges and return the error limits
...
# Select the price column and compute the 95th percentile
...
# Step 2: Further split the 80% training set into 60% training and 40% validation sets
...
y_val_pred_log = svr_model.predict(X_val_scaled)

# Inverse log transformation of the predicted values to revert to the price scale
y_val_pred = np.expm1(y_val_pred_log)
y_val = np.expm1(y_val_log)

# Ensure the predicted price lower bound is set to 0 to avoid negative values
...
y_test_pred_log = svr_model.predict(X_test_scaled)
...
plt.title('(SVM) True vs Predicted Prices SVM (Validation Set)')
plt.legend()
plt.savefig('true_vs_predicted_prices_val_svm.png')
plt.show()
...
plt.title('(SVM) True vs Predicted Prices with Reasonable Price Range (Test Set)')
plt.legend()
plt.savefig('true_vs_predicted_prices_test_svm.png')
plt.show()
