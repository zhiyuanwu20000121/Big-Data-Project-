...
# Use RandomizedSearchCV for model selection
random_search = RandomizedSearchCV(estimator=rf_model, param_distributions=param_grid, n_iter=20, cv=5, n_jobs=-1, scoring='neg_mean_squared_error', random_state=42)
...
plt.title('(Random Forest) True vs Predicted Prices(Validation Set)')
plt.legend()
plt.savefig('true_vs_predicted_prices_val_rf.png')
plt.show()
...
plt.title('(Random Forest) True vs Predicted Prices with Reasonable Price Range (Test Set)')
plt.legend()
plt.savefig('true_vs_predicted_prices_test_rf.png')
plt.show()
